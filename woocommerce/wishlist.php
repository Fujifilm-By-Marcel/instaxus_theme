        ...
        <div class="wishlist-title <?php echo ( $wishlist_meta['is_default'] != 1 && $is_user_owner ) ? 'wishlist-title-with-form' : ''?>">
            ...
            <?php if( $wishlist_meta['is_default'] != 1 && $is_user_owner ): ?>
                <a class="btn button show-title-form">
                    ...
                    <?php esc_html_e( 'Edit title', 'logancee' ) ?>
                </a>
            <?php endif; ?>
        </div>
        <?php if( $wishlist_meta['is_default'] != 1 && $is_user_owner ): ?>
    ...
    <table class="shop_table cart wishlist_table" cellspacing="0" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr( $wishlist_meta['ID'] )?>">
        ...
        <tr class="mobile-hide">
            ...
            <th class="product-wrap">
                <span class="nobr"><?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product Name', 'logancee' ) ) ?></span>
            </th>

            <?php if( $show_price ) : ?>
                <th class="product-price">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit Price', 'logancee' ) ) ?>
                    </span>
                </th>
            <?php endif ?>
            ...
            <?php if( $show_stock_status ) : ?>
                <th class="product-stock-stauts">
                    <span class="nobr">
                        <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock Status', 'logancee' ) ) ?>
                    </span>
                </th>
            <?php endif ?>
        ...
        if( count( $wishlist_items ) > 0 ) :
                        ...
                        <?php if( $show_price ) : ?>
                            <td class="product-price">
                                ...
                                if( $product->price != '0' ) {
                                    ...
                                    echo apply_filters( 'yith_free_text', esc_html__( 'Free!', 'logancee' ) );
                                }
                                ?>
                            </td>
                        <?php endif ?>
                        ...
                        <?php if( $show_stock_status ) : ?>
                            <td class="product-stock-status">
                                ...
                                if( $stock_status == 'out-of-stock' ) {
                                    $stock_status = "Out";
                                    echo '<span class="wishlist-out-of-stock">' . esc_html__( 'Out of Stock', 'logancee' ) . '</span>';
                                } else {
                                    $stock_status = "In";
                                    echo '<span class="wishlist-in-stock">' . esc_html__( 'In Stock', 'logancee' ) . '</span>';
                                }
                                ?>
                            </td>
                        <?php endif ?>
                            ...
                            <td class="product-add-to-cart">
                                ...
                                <?php if( $is_user_owner ): ?>
                                    <div class="product-remove">
                                        <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item['prod_id'] ) ) ?>" class="btn-arrow remove remove_from_wishlist remove-icon" title="<?php esc_html_e( 'Remove this product', 'logancee' ) ?>">&times;</a>
                                    </div>
                                <?php endif; ?>
                            </td>
                        <?php endif ?>
            ...
            <tr class="pagination-row">
                <td colspan="6" class="wishlist-empty"><?php esc_html_e( 'No products were added to the wishlist', 'logancee' ) ?></td>
            </tr>
        <?php
        endif;
    ...
    <?php if( $is_user_logged_in ): ?>
        ...
        if ( $is_user_owner && $show_ask_estimate_button && $count > 0 ): ?>
            <a href="<?php echo $ask_estimate_url ?>" class="btn button ask-an-estimate-button">
                ...
                <?php esc_html_e( 'Ask an estimate of costs', 'logancee' ) ?>
            </a>
        <?php
        endif;
